...
char tipo_casella(record_partita partita, int indice_giocatore) {
    ...
    giocatore = leggere_giocatore_record_vet_giocatori(partita, indice_giocatore);
    numero_casella = leggere_posizione_record_giocatore(giocatore);
    percorso = leggere_percorso_record_partita(partita);
    char tipo_casella = leggere_casella_record_percorso(percorso, numero_casella);

    return tipo_casella;
}

record_partita applicare_effetto_casella_labirinto(record_partita partita, int indice_giocatore) {
    record_percorso percorso = leggere_record_percorso_record_partita(partita);

    int dimensione_percorso = percorso.dimensione;

    int nuova_posizione = calcolare_proporzione(ARRIVO_LABIRINTO, NUMERO_MASSIMO_CASELLE, dimensione_percorso);

    record_giocatore* vet_giocatori = leggere_vet_giocatori_record_partita(partita);

    record_giocatore giocatore = leggere_giocatore_record_vet_giocatori(vet_giocatori, indice_giocatore);

    giocatore.numero_casella = nuova_posizione;
...
record_partita applicare_effetto_casella_prigione(record_partita partita, int indice_giocatore) {
    int NUMERO_MASSIMO_GIOCATORI = 4;
    int PRIMO_INDICE_ARRAY = 0;
    int CASELLA_PRIGIONE = 'P';
    ...
    while (i < NUMERO_MASSIMO_GIOCATORI && !trovato) {
        record_giocatore giocatore = leggere_giocatore_record_vet_giocatori(vet_giocatori, i);

        if (trovare_tipo_casella_giocatore(giocatore) == CASELLA_PRIGIONE && leggere_bloccato_record_giocatore(giocatore)) {
            giocatore = scrivere_bloccato_giocatore(giocatore, 0);
    ...
    record_giocatore giocatore = leggere_giocatore_record_vet_giocatori(vet_giocatori, indice_giocatore);
    giocatore = scrivere_bloccato_giocatore(giocatore, 1);
    vet_giocatori = scrivere_giocatore_record_vet_giocatori(vet_giocatori, indice_giocatore, giocatore);
...
record_partita applicare_effetto_casella_scheletro(record_partita partita, int indice_giocatore) {
    int PRIMA_POSIZIONE_PERCORSO = 1;

    record_giocatore* vet_giocatori = leggere_vet_giocatori_record_partita(partita);
    record_giocatore giocatore = leggere_giocatore_record_vet_giocatori(vet_giocatori, indice_giocatore);

    giocatore.numero_casella = PRIMA_POSIZIONE_PERCORSO;
...
record_partita giocare_partita(record_partita partita) {
    while (!verificare_partita_iniziata(partita)) {
    ...
    while (!leggere_terminata_record_partita(partita) &&
        ...
        int indice_giocatore_di_turno = leggere_indice_giocatore_di_turno_record_partita(partita);
        stampare_file_di_testo(menu_partita);
        int scelta = chiedere_intero("Inserisci la scelta: ", 0, 3);
